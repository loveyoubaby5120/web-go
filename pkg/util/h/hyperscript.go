...
var (
	sanitizePolicy = func() *bluemonday.Policy {
		...
		p.AllowAttrs("data-indent").OnElements("p")
		return p
	}()
)

type SanitizeHTML string

func (s SanitizeHTML) ICanBeToken() {}

func (s SanitizeHTML) ToHTML(w StringWriter) error {
	w.WriteString(sanitizePolicy.Sanitize(string(s)))
	return nil
}
...
func MustToString(n N) string {
	result, err := ToString(n)
	if err != nil {
		panic(err)
	}
	return result
