...
// SortMapByValue converts a map into a PairList, then sort and return it.
...
// StringSetToSlice returns sorted []string in increasing order
func StringSetToSlice(m map[string]bool) []string {
	result := StringSetToSliceNoSort(m)
	sort.Strings(result)
	return result
}

func StringSetToSliceNoSort(m map[string]bool) []string {
	var result []string
	for k, v := range m {
		if v {
